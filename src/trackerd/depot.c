...
int dpgetflags(DEPOT *depot){
  assert(depot);
  return (*(unsigned char *)(depot->map + DP_FLAGSOFF + 0) << 0) +
    (*(unsigned char *)(depot->map + DP_FLAGSOFF + 1) << 8) +
    (*(unsigned char *)(depot->map + DP_FLAGSOFF + 2) << 16) +
    (*(unsigned char *)(depot->map + DP_FLAGSOFF + 3) << 24);
}
...
int dpsetflags(DEPOT *depot, int flags){
  ...
  *(depot->map + DP_FLAGSOFF + 0) = (flags >> 0) & 0xff;
  *(depot->map + DP_FLAGSOFF + 1) = (flags >> 8) & 0xff;
  *(depot->map + DP_FLAGSOFF + 2) = (flags >> 16) & 0xff;
  *(depot->map + DP_FLAGSOFF + 3) = (flags >> 24) & 0xff;
  return TRUE;
}
