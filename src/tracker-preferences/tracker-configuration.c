		       ...
		       const gchar * key, GError ** error)
{
	...
	value = g_key_file_get_value (key_file, group_name, key,
				      &key_file_error);
...
create_config_file ()
{
	...
	if (!g_file_test (filename, G_FILE_TEST_EXISTS)) {
		...
		g_file_set_contents (filename, contents, strlen (contents),
				     NULL);
		g_free (contents);
	}
...
GSList *tracker_configuration_get_list (TrackerConfiguration * configuration,
					const gchar * const key,
...
*/
	char *my_contents =
		g_key_file_to_data (priv->keyfile, &length, &error);
...
static void
_set_string (TrackerConfiguration * configuration,
...
static GSList *_get_string_list (TrackerConfiguration * configuration,
				 const gchar * const key,
				 GError ** error)
{
	TrackerConfiguration *self = TRACKER_CONFIGURATION (configuration);
